...
import type {
  CodaInternalRequestInterceptor,
  CodaInternalRequestConfig,
  CodaRequestConfig
} from './type'
...
class CodaRequest {
  ...
  interceptors?: CodaInternalRequestInterceptor
  // showLoading: boolean

  constructor(config: CodaInternalRequestConfig) {
    // 创建axios实例
  ...
  // 封装request方法
  request<T>(config: CodaRequestConfig<T>): Promise<T> {
  ...
  get<T>(config: CodaRequestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: 'GET' })
  }
  post<T>(config: CodaRequestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: 'POST' })
  }
  delete<T>(config: CodaRequestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: 'DELETE' })
  }
  patch<T>(config: CodaRequestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: 'PATCH' })
  }
}

export default CodaRequest
